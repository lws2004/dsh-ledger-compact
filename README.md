...
- **A different glyph design at the same size is neutral**: four atlases measured (X.org 8x13 and TTF renders of DejaVu/JetBrains). The screening put DejaVu Bold three answers ahead; the paired 3-repeat confirmation came out **46/60 vs 46/60**. The encoder reads pixels per glyph, not penmanship. `tools/make-atlas.py` ships, so the untested half — a *larger* glyph (10x16 in a 10x18 cell) — is one bench run away.
