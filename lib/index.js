...
import { hookMechanicalSummarizer, setReplaceDefault, withMechanicalFold } from "./hook.js";
...
function apply(ctx) {
  ...
  const service = {
    ...
    async getStatus(sessionId) {
      ...
      hookKnownEngines(ctx, session ? { session, options: session.requestHeader?.()?.config ?? {}, ctx: session.ctx } : undefined);
      if (!session) {
...
export { apply, inject, name, parseCommand, hookMechanicalSummarizer, setReplaceDefault, withMechanicalFold, resolveCompactionEngine, collectCompactionEngines };
