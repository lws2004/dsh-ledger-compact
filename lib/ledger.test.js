...
import { excerptNotice, isPlaceholder, isPersistReadback, isRereadableTool, isSkillLoad, persistIngressText, persistRetentionLabel, snapExcerpt, collectText, tokenDigest, toolSourceLabel, PERSIST_DIR_DEFAULT, PERSIST_TTL_MS, readPersistText, skipIngressTool } from "./excerpt.js";
import { retrieveToolDefinition } from "./retrieve-tool.js";
import { shapeIngress, shapeCurrentTurnIngress, resetReplaceOpShape, usesLegacyReplaceKeys } from "./ingress.js";
import { buildFold, foldSummary, foldSummaryDecided, foldReport, inspectMessages, carriedFromFold, formatFoldNotice, formatTokenCount, collectSummaryText, parseFoldNotice, FOLD_CARD_MARKER } from "./fold.js";
import { apply, parseCommand } from "./index.js";
...
import { verdictAction, parseBatch, pickCandidates, decideEntries, summarizeVerdicts, describeVerdicts, verdictState, DECIDE_FOLD_DEFAULTS } from "./decide-fold.js";
...
test("the re-read promise is only made where it can be kept", () => {
  const lines = Array.from({ length: 60 }, (_, i) => "L" + i).join("\n");
  const readable = snapExcerpt(lines, { rereadable: true });
  const notReadable = snapExcerpt(lines, { rereadable: false });
  assert.ok(readable.includes("re-read with offset/limit"));
  assert.equal(notReadable.includes("offset/limit"), false);
  assert.ok(notReadable.includes("cannot be re-read"));
  // 默认保持旧措辞，别的调用点不受影响
  assert.ok(snapExcerpt(lines).includes("re-read with offset/limit"));
});

test("a long-line result keeps the notice that says it was cut", () => {
  // 165 字符/行时，头 16 行就超过 2400 字节上限：窗口先定尺寸的话，从尾部截断会把注记
  // 连同尾部一起切掉，模型拿到的是一段"看起来完整"的开头。
  const lines = Array.from({ length: 200 }, (_, i) => "line-" + i + " " + "abcd".repeat(40));
  const out = snapExcerpt(lines.join("\n"));
  assert.ok(out.includes("lines elided"), "注记必须在场");
  assert.ok(Buffer.byteLength(out, "utf8") <= 2400);
  assert.ok(out.startsWith("line-0 "), "头部仍在");
  assert.ok(out.includes("line-19"), "尾部仍在");
});

test("isRereadableTool only trusts a file read", () => {
  assert.equal(isRereadableTool("read"), true);
  assert.equal(isRereadableTool("READ"), true);
  assert.equal(isRereadableTool("run_code"), false);
  assert.equal(isRereadableTool("bash"), false);
  assert.equal(isRereadableTool("web_fetch"), false);
  assert.equal(isRereadableTool(undefined), false);
});

test("shapeIngress stops promising a re-read for command output", () => {
  // 行要短到让摘录落在 2400 字节上限内：更长的行会把省略注记整个挤掉（那是另一个问题）。
  const text = Array.from({ length: 200 }, (_, i) => "line-" + i + " " + "ab".repeat(10)).join("\n");
  const viaRead = shapeIngress(text, { vision: false, minSnapTokens: 100, toolName: "read" });
  const viaRun = shapeIngress(text, { vision: false, minSnapTokens: 100, toolName: "run_code" });
  assert.ok(viaRead.text.includes("re-read with offset/limit"));
  assert.equal(viaRun.text.includes("offset/limit"), false);
  assert.ok(viaRun.text.includes("cannot be re-read"));
});
...
test("toolSourceLabel names the call and the argument that locates it", () => {
  assert.equal(toolSourceLabel("read", { file_path: "src/a.ts" }), "read src/a.ts");
  assert.equal(toolSourceLabel("bash", { command: "ls -la\n  extra" }), "bash ls -la extra");
  assert.equal(toolSourceLabel("grep", { pattern: "needle", path: "src" }), "grep src");
  assert.equal(toolSourceLabel("read", undefined), "read");
  assert.equal(toolSourceLabel("", { path: "src" }), "src");
  assert.equal(toolSourceLabel("read", { unrelated: 1 }), "read");
  // The weakest locator, and the only one a wrapper tool can offer.
  assert.equal(toolSourceLabel("run_code", { description: "Read failing test" }), "run_code Read failing test");
  assert.equal(toolSourceLabel("run_code", { code: "x".repeat(500) }), "run_code");
});

test("toolSourceLabel flattens and bounds an overlong argument", () => {
  const label = toolSourceLabel("read", { file_path: "x".repeat(200) });
  assert.ok(label.startsWith("read "));
  assert.ok(label.length <= 65);
  assert.ok(label.endsWith("…"));
});

test("excerptNotice carries a source label and is unchanged without one", () => {
  assert.equal(excerptNotice(9000, "body", "", "").split("\n")[0], "[Snapcompact: 9000 tokens → excerpt]");
  assert.equal(
    excerptNotice(9000, "body", "", "", "read src/a.ts").split("\n")[0],
    "[Snapcompact: 9000 tokens → excerpt · read src/a.ts]"
  );
  assert.equal(
    excerptNotice(9000, "body", "1024x624 PNG ~540 tokens", "", "read src/a.ts").split("\n")[0],
    "[Snapcompact: 9000 tokens → 1024x624 PNG ~540 tokens · read src/a.ts]"
  );
});

test("shapeIngress puts the source label on the excerpt it emits", () => {
  const text = Array.from({ length: 200 }, (_, i) => "line-" + i + " " + "abcd".repeat(40)).join("\n");
  const shaped = shapeIngress(text, { vision: false, minSnapTokens: 100, source: "read src/a.ts" });
  assert.equal(shaped.changed, true);
  assert.ok(shaped.text.split("\n")[0].includes("· read src/a.ts"));
  assert.ok(shaped.tokensSaved > 0);
});

test("foldSummary reports the fold through rawOutput without claiming an LLM call", () => {
  const result = foldSummary({
    messages: [user("Ship it"), assistant([toolCall("c1", "read", { file_path: "src/a.ts" })]), toolResult("c1", "body")]
  });
  assert.equal(result.llmStreamCall, undefined);
  assert.equal(Array.isArray(result.rawOutput), true);
  assert.equal(result.rawOutput.length, 2);
  assert.ok(result.rawOutput[0].text.startsWith("dsh-ledger-compact fold"));
  const payload = JSON.parse(result.rawOutput[1].text);
  assert.equal(payload.spanMessages, 3);
  assert.equal(payload.reads, 1);
  assert.deepEqual(payload.tools, [{ name: "read", n: 1 }]);
  assert.ok(payload.discardedTokens > 0);
  assert.ok(payload.cardLines > 0);
});

test("foldReport is exported and honours an explicit token count", () => {
  const report = foldReport([user("goal"), toolResult("c9", "body")], "card", { discardedTokens: 12 });
  assert.equal(JSON.parse(report[1].text).discardedTokens, 12);
});

test("foldSummary can skip the report when only the summary is wanted", () => {
  const result = foldSummary({ messages: [user("Ship it")] }, { report: false });
  assert.equal(result.rawOutput, undefined);
});

test("apply keeps shaping when the settings layer cannot be registered", async () => {
  const warnings = [];
  let service;
  const ctx = {
    settings: {
      register() {
        throw new Error("namespace dsh-ledger-compact is already registered");
      }
    },
    provide: (name, value) => { service = value; },
    typert: { register: () => {} },
    commands: { register: () => () => {} },
    tools: { register: () => () => {} },
    on: () => () => {},
  ...
  assert.ok(warnings[0].includes("settings layer unavailable"));
  const health = service.getHealth();
  assert.equal(health.settings.ok, false);
  assert.ok(health.settings.error.includes("already registered"));
  // Defaults still drive shaping, and a save reports the degradation instead of throwing.
  assert.equal(health.config.enabled, true);
  assert.equal(health.config.replaceDefault, false);
  await assert.rejects(() => service.saveConfig({ enabled: false }), /settings layer unavailable/);
});
...
/** The message-producing types a DSH surface folds. A `tool/call` is log-only. */
...
function fakeSession(events, options = {}) {
  ...
  // The real surface folds message-producing events only: a `tool/call` stays in the log
  // and never becomes a node, which is exactly what a tool-name lookup has to survive.
  const nodes = events.filter((event) => SURFACE_EVENT_TYPES.has(event.type)).map((event) => event.seq);
  const surface = { nodes };
  return {
    surface,
    log,
...
/**
 * Drive one ingress pass over a single run_code call, the way this deployment delivers it.
 *
 * The program the call runs is what the pass inspects, so each case states its program and the
 * fate it expects; the session, the meter and the step arithmetic are the same every time.
 */
async function runCodeIngress(code, description) {
  resetReplaceOpShape();
  const session = fakeSession([
    { seq: 0, type: "tool/call", data: { turn: 1, step: 1, callId: "c1", name: "run_code", arguments: JSON.stringify({ code, description }) } },
    { seq: 1, type: "tool/result", data: { turn: 1, step: 1, message: toolResult("c1", BIG_TEXT) } }
  ]);
  const ctx = { get: (name) => (name === "tokenMeter" ? { estimateMessage: () => 4242 } : undefined) };
...
test("a call that reads back a persisted result is never shaped again", async () => {
  const { session, report } = await runCodeIngress(
    'await tools.snap_retrieve({ hash: "898c69ab8636" })',
    "Read back the elided bytes"
  );
  assert.equal(report.shaped, 0, "the read-back keeps every byte");
  assert.equal(report.failed, 0);
  assert.equal(session.log.length, 2, "no rewrite event was appended");
  assert.equal(textOfEvent(session.eventAt(1)), BIG_TEXT);
});

test("a run_code call that does not name the persist directory is still shaped", async () => {
  const { report } = await runCodeIngress("await tools.bash({ command: 'ls src' });", "List sources");
  assert.equal(report.shaped, 1, "an ordinary call is shaped as before");
  assert.equal(report.failed, 0);
});

test("a run_code call that loads a skill keeps every byte", async () => {
  const { session, report } = await runCodeIngress(
    'const s = await tools.skill({ name: "typed-decide" }); console.log(s.content);',
    "Load typed-decide skill"
  );
  assert.equal(report.shaped, 0, "the skill body keeps every byte");
  assert.equal(report.failed, 0);
  assert.equal(session.log.length, 2, "no rewrite event was appended");
  assert.equal(textOfEvent(session.eventAt(1)), BIG_TEXT);
});

test("the retention label reads as a span an operator can act on", () => {
  assert.equal(persistRetentionLabel(PERSIST_TTL_MS), "24h");
  assert.equal(persistRetentionLabel(30 * 60 * 1000), "30m");
  assert.equal(persistRetentionLabel(3 * 24 * 60 * 60 * 1000), "3d");
});

test("the notice names a redeem handle instead of a path", () => {
  const text = Array.from({ length: 300 }, (_, i) => "line-" + i + " " + "x".repeat(60)).join(String.fromCharCode(10));
  const excerpt = snapExcerpt(text, { savedHash: "7f3a91c2", rereadable: false, persistTtlMs: PERSIST_TTL_MS });
  assert.match(excerpt, /snap_retrieve\("7f3a91c2"\)/, "the marker names the call that redeems it");
  assert.equal(/Full text saved to/.test(excerpt), false, "no absolute path is spent on every request");
});

test("expired copies are dropped even when the directory is far below its cap", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-ttl-"));
  try {
    // A copy old enough to expire, and one written now.
    const stale = join(dir, "stale-call.txt");
    writeFileSync(stale, "old bytes", "utf8");
    const past = new Date(Date.now() - 48 * 60 * 60 * 1000);
    utimesSync(stale, past, past);
    const fresh = persistIngressText("new bytes", dir, "fresh", 60 * 60 * 1000);
    assert.ok(fresh, "the fresh copy is written");

    // prune runs every 25th write; drive it to the threshold with expired files.
    for (let i = 0; i < 25; i += 1) persistIngressText("x", dir, "filler" + i, 60 * 60 * 1000);
    const left = readdirSync(dir);
    assert.equal(left.includes("stale-call.txt"), false, "the expired copy is gone");
    assert.equal(left.includes(fresh.split("/").pop()), true, "the fresh copy survives");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("isPersistReadback recognises the redeem call and nothing else", () => {
  assert.equal(isPersistReadback({ code: "await tools.snap_retrieve({ hash: \"7f3a91c2\" })" }), true, "the redeem call is recognised by tool name");
  assert.equal(isPersistReadback({ code: "grep -rn ledger-ingress docs/" }), false, "a bare directory mention is not a redemption");
  assert.equal(isPersistReadback({ file_path: "/tmp/elsewhere/abc.txt" }), false);
  assert.equal(isPersistReadback({}), false);
  assert.equal(isPersistReadback(undefined), false);
});

test("isSkillLoad recognises the call that pulls a skill in through the ptc wrapper", () => {
  assert.equal(isSkillLoad({ code: 'const s = await tools.skill({ name: "typed-decide" });' }), true, "the documented call shape");
  assert.equal(isSkillLoad({ code: 'await tools["open_skill"]({ name: "x" })' }), true, "bracket access is the same call");
  assert.equal(isSkillLoad({ code: "const s = await tools.open_skill({ name: 'x' });" }), true);
  assert.equal(isSkillLoad({ code: "await tools.find_skills({ query: 'pdf' })" }), true, "the catalogue is instruction text too");
  assert.equal(isSkillLoad({ code: "await tools.bash({ command: 'ls src' });" }), false, "an ordinary call is shaped as before");
  assert.equal(isSkillLoad({ code: "await tools.read({ file_path: '/tmp/a.md' })" }), false);
  assert.equal(isSkillLoad({ file_path: "/tmp/elsewhere/abc.txt" }), false);
  assert.equal(isSkillLoad({}), false);
  assert.equal(isSkillLoad(undefined), false);
});

test("a redeemed copy is skipped by name, the way skill and context are", () => {
  assert.equal(skipIngressTool("snap_retrieve"), true, "native mode delivers the redemption under its own name");
  assert.equal(skipIngressTool("skill"), true);
  assert.equal(skipIngressTool("open_skill"), true, "the loader the catalogue points at is the same contract");
  assert.equal(skipIngressTool("find_skills"), true);
  assert.equal(skipIngressTool("read"), false);
});

test("ingress exempts skill and context results whose tool/call never reaches the surface", async () => {
  resetReplaceOpShape();
  for (const tool of ["skill", "context"]) {
    const session = fakeSession([
      { seq: 0, type: "tool/call", data: { turn: 1, step: 1, callId: "c1", name: tool, arguments: "{}" } },
      { seq: 1, type: "tool/result", data: { turn: 1, step: 1, message: toolResult("c1", BIG_TEXT) } }
    ]);
    assert.deepEqual(session.surface.nodes, [1], "a tool/call is log-only, never a surface node");
    const ctx = { get: (service) => (service === "tokenMeter" ? { estimateMessage: () => 4242 } : undefined) };
    const report = await shapeCurrentTurnIngress(session, ctx, { turn: 1, step: 2, vision: false, minSnapTokens: 100 });
    assert.equal(report.failed, 0);
    assert.equal(report.shaped, 0, tool + " results are never shaped");
    assert.equal(session.log.some((event) => event.type === "compaction/prune"), false, "nothing is shadowed");
    assert.equal(textOfEvent(session.eventAt(1)), BIG_TEXT, "the result stays byte-identical");
  }
  resetReplaceOpShape();
});

test("ingress still shapes a result whose name resolves from a log-only tool/call", async () => {
  resetReplaceOpShape();
  const session = fakeSession([
    { seq: 0, type: "tool/call", data: { turn: 1, step: 1, callId: "c1", name: "bash", arguments: "{}" } },
    { seq: 1, type: "tool/result", data: { turn: 1, step: 1, message: toolResult("c1", BIG_TEXT) } }
  ]);
  const ctx = { get: (service) => (service === "tokenMeter" ? { estimateMessage: () => 4242 } : undefined) };
  const report = await shapeCurrentTurnIngress(session, ctx, { turn: 1, step: 2, vision: false, minSnapTokens: 100 });
  assert.equal(report.shaped, 1, "a non-exempt tool keeps its ingress shaping");
  const replacement = session.log.find((event) => event.type === "tool/result" && event.surfaceOp?.op === "replace");
  assert.ok(textOfEvent(replacement).startsWith("[Snapcompact"));
  resetReplaceOpShape();
});

test("ingress shapes a v4 first-class tool-role result that carries no tool-result wrapper", async () => {
  resetReplaceOpShape();
  // Format v4 lifts a tool result into a `role: "tool"` message whose content is plain
  // blocks; the v3 wrapper is gone, so a wrapper-only lookup silently matched nothing.
  const session = fakeSession([
    { seq: 0, type: "tool/call", data: { turn: 1, step: 1, callId: "c1", name: "bash", arguments: "{}" } },
    {
      seq: 1,
  ...
  assert.equal(report.shaped, 1, "a v4 tool-role result must still be shaped");
  const replacement = session.log.find((event) => event.type === "tool/result" && event.surfaceOp?.op === "replace");
  assert.ok(replacement, "the rewrite lands as a replace surfaceOp");
  assert.ok(textOfEvent(replacement).startsWith("[Snapcompact"));
  resetReplaceOpShape();
});

test("ingress leaves instruction text whole even on a non-exempt tool", async () => {
  resetReplaceOpShape();
  const session = fakeSession([
    { seq: 0, type: "tool/call", data: { turn: 1, step: 1, callId: "c1", name: "bash", arguments: "{}" } },
    { seq: 1, type: "tool/result", data: { turn: 1, step: 1, message: toolResult("c1", '<skill_content name="x">\n' + BIG_TEXT) } }
  ]);
  const ctx = { get: (service) => (service === "tokenMeter" ? { estimateMessage: () => 4242 } : undefined) };
  const report = await shapeCurrentTurnIngress(session, ctx, { turn: 1, step: 2, vision: false, minSnapTokens: 100 });
  assert.equal(report.shaped, 0, "instruction text is a command to follow, not data to sample");
  resetReplaceOpShape();
});
...
test("apply wires the service, the command and the pre-step health counters", async () => {
  ...
  const ctx = {
    ...
    tools: { register: (tool) => { registered.tool = tool; return () => {}; } },
...
test("the canonical envelope rehydrates in the real schemastery", async (t) => {
  ...
  const reference = schemaLib.object({
    ...
    decideFold: schemaLib.boolean().default(false),
    persistIngress: schemaLib.boolean().default(false),
    persistDir: schemaLib.string().default(""),
    persistTtlHours: schemaLib.number().step(1).min(0).max(8760).default(24)
  });
...
const BIG_RESULT = Array.from({ length: 120 }, (_, i) => "setting_" + i + " = " + ((i * 7) % 100)).join("\n");

function verdictMessages() {
  return [
    user("tighten the retry policy in the config module"),
    assistant([toolCall("c1", "read", { file_path: "src/config.ts" })]),
    toolResult("c1", BIG_RESULT),
  ];
}

test("the verdict fold is off by default: the card is exactly the mechanical one", () => {
  const messages = verdictMessages();
  const plain = foldSummary({ messages }).summary[0].text;
  assert.equal(plain.includes("CARRIED DETAIL"), false);
  const snapshot = inspectMessages(messages);
  assert.equal(snapshot.entries.length, 1);
  assert.equal(snapshot.entries[0].target, "src/config.ts");
  assert.equal(snapshot.entries[0].kind, "read");
  assert.ok(snapshot.entries[0].text.includes("setting_0 = 0"));
});

test("a keep verdict restores the body the mechanical card drops", () => {
  const messages = verdictMessages();
  const verdicts = new Map([["c1", { action: "keep", pNeeded: 0.94, pRerun: 0.03, neededMargin: 0.8, rerunMargin: 0.7 }]]);
  const text = foldSummary({ messages }, { verdicts }).summary[0].text;
  assert.ok(text.includes("CARRIED DETAIL"), text);
  assert.ok(text.includes("setting_3 = 21"), "the body comes back verbatim");
  assert.ok(text.includes("still needed, not reproducible"));
});

test("only a keep verdict adds body: drop and truncate leave the card alone", () => {
  const messages = verdictMessages();
  for (const action of ["drop", "truncate", "mechanical"]) {
    const verdicts = new Map([["c1", { action }]]);
    const text = foldSummary({ messages }, { verdicts }).summary[0].text;
    assert.equal(text.includes("CARRIED DETAIL"), false, action);
  }
});

test("the quadrant needs confidence on both questions", () => {
  const base = { neededMargin: 0.6, rerunMargin: 0.6 };
  assert.equal(verdictAction({ ...base, pNeeded: 0.9, pRerun: 0.1 }), "keep");
  assert.equal(verdictAction({ ...base, pNeeded: 0.1, pRerun: 0.9 }), "drop");
  assert.equal(verdictAction({ ...base, pNeeded: 0.9, pRerun: 0.9 }), "truncate");
  assert.equal(verdictAction({ ...base, pNeeded: 0.1, pRerun: 0.1 }), "keep");
  // A coin flip is not a verdict: the measured 0.02 margin is the real case here.
  assert.equal(verdictAction({ pNeeded: 0.52, pRerun: 0.84, neededMargin: 0.02, rerunMargin: 0.34 }), "mechanical");
  assert.equal(verdictAction({ ...base, pNeeded: null, pRerun: 0.9 }), "mechanical");
  assert.equal(verdictAction({ ...base, pNeeded: 0.9, pRerun: 0.9, neededMargin: null }), "mechanical");
});

test("the batch parser reads the three-layer envelope and a flat body alike", () => {
  const wrapped = parseBatch({ ok: true, result: { answers: {
    needed: { probability: 0.3, margin: 0.4 },
    rerunnable: { probability: 0.8, margin: 0.5 }
  } }, latency_ms: 12 });
  assert.deepEqual(wrapped, { needed: { probability: 0.3, margin: 0.4 }, rerun: { probability: 0.8, margin: 0.5 } });
  const flat = parseBatch({ needed: { probability: 0.7, margin: 0.2 }, rerunnable: { probability: 0.1, margin: 0.3 } });
  assert.equal(flat.needed.probability, 0.7);
  assert.equal(flat.rerun.probability, 0.1);
  assert.deepEqual(parseBatch({}), { needed: null, rerun: null });
  assert.deepEqual(parseBatch(null), { needed: null, rerun: null });
});

test("candidates are the large results, largest first", () => {
  const small = { id: "s", name: "bash", text: "ok" };
  const mid = { id: "m", name: "bash", text: BIG_RESULT };
  const big = { id: "b", name: "bash", text: BIG_RESULT.repeat(4) };
  const picked = pickCandidates([small, mid, big], { minEntryTokens: 200, maxEntries: 5 });
  assert.deepEqual(picked.map((e) => e.id), ["b", "m"]);
  assert.deepEqual(pickCandidates([small, mid, big], { minEntryTokens: 200, maxEntries: 1 }).map((e) => e.id), ["b"]);
});

test("the state carries the evidence the decider cannot derive", () => {
  const state = verdictState({ name: "read", target: "src/config.ts", text: BIG_RESULT, laterContext: "A later step (edit) rewrote the same target." },
    { task: "tighten the retry policy" });
  assert.ok(state.includes("tighten the retry policy"));
  assert.ok(state.includes("src/config.ts"));
  assert.ok(state.includes("rewrote the same target"));
});

test("a drop needs both answers; either failure keeps the mechanical card", async () => {
  const entries = [{ id: "a", name: "bash", target: "cmd", text: BIG_RESULT }];
  const answer = (needed, rerun) => async () => ({ ok: true, status: 200, json: async () => ({
    result: { answers: {
      needed: { probability: needed, margin: 0.7 },
      rerunnable: { probability: rerun, margin: 0.7 }
    } }
  }) });
  const drop = await decideEntries(entries, { key: "k", fetchImpl: answer(0.05, 0.95) });
  assert.equal(drop.get("a").action, "drop");
  const keep = await decideEntries(entries, { key: "k", fetchImpl: answer(0.95, 0.05) });
  assert.equal(keep.get("a").action, "keep");
  const lowMargin = await decideEntries(entries, { key: "k", fetchImpl: async () => ({ ok: true, status: 200, json: async () => ({
    result: { answers: { needed: { probability: 0.5, margin: 0.01 }, rerunnable: { probability: 0.99, margin: 0.6 } } }
  }) }) });
  assert.equal(lowMargin.get("a").action, "mechanical", "a coin flip never drops context");
});

test("an unreachable, credential-less or slow decider costs verdicts, never the fold", async () => {
  const entries = [{ id: "a", name: "bash", text: BIG_RESULT }];
  const boom = await decideEntries(entries, { key: "k", fetchImpl: async () => { throw new Error("ECONNREFUSED"); } });
  assert.equal(boom.get("a").action, "mechanical");
  assert.ok(boom.get("a").error.includes("ECONNREFUSED"));
  const noKey = await decideEntries(entries, { key: "", fetchImpl: async () => { throw new Error("should not run"); } });
  assert.equal(noKey.get("a").action, "mechanical");
  assert.equal(noKey.get("a").error, "no decider credential");
  const hang = await decideEntries(entries, { key: "k", budgetMs: 200, fetchImpl: () => new Promise(() => {}) });
  assert.equal(hang.get("a").action, "mechanical");
  assert.equal(hang.get("a").error, "budget exceeded");
});

test("the fold orchestration degrades to the mechanical card with no credential", async () => {
  const previous = process.env.LITELLM_MASTER_KEY;
  delete process.env.LITELLM_MASTER_KEY;
  try {
    const messages = verdictMessages();
    const decided = await foldSummaryDecided({ messages }, { minEntryTokens: 200 });
    assert.equal(decided.summary[0].text.includes("CARRIED DETAIL"), false);
    assert.equal(decided.summary[0].text, foldSummary({ messages }).summary[0].text);
  } finally {
    if (previous !== undefined) process.env.LITELLM_MASTER_KEY = previous;
  }
});

test("a span with no user request skips the verdict pass entirely", async () => {
  // Measured on three real sessions: handed no task, the decider answered "drop" for all
  // twelve candidates — byte-identical to the empty-task answer. "Still needed to finish
  // the current task" has no referent without a request, so no call is spent at all.
  const messages = verdictMessages().filter((m) => m.role !== "user");
  let calls = 0;
  const decided = await foldSummaryDecided({ messages }, {
    minEntryTokens: 200,
    key: "k",
    fetchImpl: async () => { calls += 1; throw new Error("the pass must not run"); }
  });
  assert.equal(calls, 0);
  assert.equal(decided.summary[0].text, foldSummary({ messages }).summary[0].text);
});

test("the fold report names the verdict pass for the trajectory tab", () => {
  const messages = verdictMessages();
  const verdicts = new Map([["c1", { action: "keep" }], ["c2", { action: "drop" }], ["c3", { action: "mechanical", error: "budget exceeded" }]]);
  verdicts.elapsedMs = 1234;
  const blocks = foldReport(messages, "card", { verdicts });
  assert.ok(blocks[0].text.includes("verdict fold"));
  assert.ok(blocks[0].text.includes("3 asked · keep 1 · truncate 0 · drop 1 · mechanical 1 · failed 1 · 1234ms"));
  const payload = JSON.parse(blocks[1].text);
  assert.equal(payload.verdictFold.asked, 3);
  assert.equal(payload.verdictFold.keep, 1);
  assert.equal(payload.verdictFold.elapsedMs, 1234);
  assert.equal(describeVerdicts(new Map()).length > 0, true);
  assert.equal(summarizeVerdicts(new Map()).asked, 0);
});

test("the evidence sentence states absence instead of arguing for a drop", async () => {
  // Measured on a real 251-result session: the earlier wording ("nothing in the span refers
  // back to this result") produced 10 drops of 12, and the neutral one produced 1. The
  // decider was answering the hint, so the hint is what the test pins.
  const captured = [];
  const fetchImpl = async (url, init) => {
    captured.push(JSON.parse(init.body).state);
    return { ok: true, status: 200, json: async () => ({ result: { answers: {
      needed: { probability: 0.1, margin: 0.9 },
      rerunnable: { probability: 0.9, margin: 0.9 }
    } } }) };
  };
  await decideEntries([
    { id: "a", name: "run_code", text: BIG_RESULT, target: "" },
    { id: "b", name: "read", text: BIG_RESULT, target: "src/config.ts" }
  ], { key: "k", fetchImpl });
  assert.equal(captured.length, 2);
  for (const state of captured) {
    assert.equal(/refers back to this result/.test(state), false, state);
    assert.equal(/only record of it/.test(state), false, state);
  }
  assert.ok(captured[0].includes("does not say whether it has been superseded"));
  assert.ok(captured[1].includes("No later step in the span touches the same target."));
});

test("the pass bounds its concurrency and retries a transient failure exactly once", async () => {
  let peak = 0;
  let live = 0;
  let calls = 0;
  ...
  const entries = Array.from({ length: 10 }, (_, i) => ({ id: "e" + i, name: "bash", text: BIG_RESULT }));
  const out = await decideEntries(entries, { key: "k", fetchImpl: slow, concurrency: 3 });
  assert.equal(out.size, 10);
  assert.equal(calls, 10);
  assert.ok(peak <= 3, "peak concurrency was " + peak);
  // Twelve at once measured worse than four, not faster: the default must stay bounded.
  assert.ok(DECIDE_FOLD_DEFAULTS.concurrency < DECIDE_FOLD_DEFAULTS.maxEntries);

  let attempts = 0;
  const flaky = async () => {
    attempts += 1;
    if (attempts === 1) throw new Error("decider HTTP 429");
    return { ok: true, status: 200, json: async () => answer };
  };
  const single = await decideEntries([{ id: "x", name: "bash", text: BIG_RESULT }],
    { key: "k", fetchImpl: flaky, retryDelayMs: 1 });
  assert.equal(single.get("x").action, "drop");
  assert.equal(attempts, 2, "one retry, not more");
});

test("a later step on the same target is stated as fact, and a rewrite is distinguished", async () => {
  const captured = [];
  const fetchImpl = async (url, init) => {
    captured.push(JSON.parse(init.body).state);
    return { ok: true, status: 200, json: async () => ({ result: { answers: {
      needed: { probability: 0.1, margin: 0.9 }, rerunnable: { probability: 0.9, margin: 0.9 }
    } } }) };
  };
  await decideEntries([
    { id: "a", name: "read", text: BIG_RESULT, target: "src/config.ts", kind: "read" },
    { id: "b", name: "edit", text: BIG_RESULT, target: "src/config.ts", kind: "edit" }
  ], { key: "k", fetchImpl });
  assert.ok(captured[0].includes("A later step (edit) rewrote the same target"));
});

const BIG_CMD = Array.from({ length: 400 }, (_, i) => "line " + i + " of a long command output").join(String.fromCharCode(10));

test("an unreadable result is written down so the notice names a reachable path", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-persist-"));
  const shaped = shapeIngress(BIG_CMD, {
    toolName: "run_code",
    minSnapTokens: 100,
    persistIngress: true,
    persistDir: dir,
  ...
  assert.equal(shaped.changed, true);
  assert.ok(shaped.savedPath, "the shape reports where the bytes went");
  assert.match(shaped.savedHash, /^[a-f0-9]{12}$/, "the handle is a short opaque name");
  assert.ok(shaped.text.includes("snap_retrieve(\"" + shaped.savedHash + "\")"), "the notice names the redeem call");
  assert.equal(readFileSync(shaped.savedPath, "utf8"), BIG_CMD, "the saved file is the original text");
  assert.equal(/cannot be re-read/.test(shaped.text), false, "the dead-end wording is gone once a copy exists");
  rmSync(dir, { recursive: true, force: true });
});

test("persistence stays off unless the caller asks for it", () => {
  const shaped = shapeIngress(BIG_CMD, { toolName: "run_code", minSnapTokens: 100 });
  assert.equal(shaped.changed, true);
  assert.equal(shaped.savedPath, undefined);
  assert.ok(/cannot be re-read/.test(shaped.text), "without a saved path the notice still tells the truth");
});

test("a readable tool keeps its offset/limit contract and writes nothing", () => {
  const shaped = shapeIngress(BIG_CMD, { toolName: "read", minSnapTokens: 100, persistIngress: true });
  assert.equal(shaped.savedPath, undefined);
  assert.ok(shaped.text.includes("re-read with offset/limit"));
});

test("persistIngressText names each copy by a short opaque handle", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-persist-"));
  const file = persistIngressText("hello" + String.fromCharCode(10) + "world", dir, "call:with/slashes");
  assert.ok(file.startsWith(dir));
  assert.equal(readFileSync(file, "utf8"), "hello" + String.fromCharCode(10) + "world");
  const name = file.slice(dir.length + 1);
  assert.match(name, /^[a-f0-9]{12}\.txt$/, "the name is a handle, not a path fragment");
  const second = persistIngressText("other bytes", dir, "call:with/slashes");
  assert.notEqual(second, file, "two copies written in the same millisecond never collide");
  rmSync(dir, { recursive: true, force: true });
});

test("readPersistText returns the bytes, and says why when it cannot", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-read-"));
  try {
    const path = persistIngressText("the original bytes", dir, "c1");
    const hash = path.slice(dir.length + 1, -4);
    const hit = readPersistText(hash, dir);
    assert.equal(hit.ok, true);
    assert.equal(hit.text, "the original bytes");
    const unknown = readPersistText("ffffffffffff", dir);
    assert.equal(unknown.ok, false);
    assert.match(unknown.reason, /no copy/, "an unknown handle says what is missing");
    assert.match(readPersistText("../etc/passwd", dir).reason, /not a valid handle/, "a traversal attempt is refused, not resolved");
    assert.equal(readPersistText("", dir).ok, false);
    assert.equal(readPersistText(undefined, dir).ok, false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("an expired copy explains itself instead of reading as a wrong handle", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-exp-"));
  try {
    const path = persistIngressText("old bytes", dir, "c1");
    const hash = path.slice(dir.length + 1, -4);
    const past = new Date(Date.now() - 48 * 60 * 60 * 1000);
    utimesSync(path, past, past);
    const miss = readPersistText(hash, dir, 24 * 60 * 60 * 1000);
    assert.equal(miss.ok, false);
    assert.match(miss.reason, /expired/);
    assert.match(miss.reason, /kept 24h/);
    assert.equal(readPersistText(hash, dir, 0).ok, true, "a zero TTL disables expiry");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("the redeem tool carries its own usage instructions", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-tool-"));
  try {
    const path = persistIngressText("the original bytes", dir, "c1");
    const hash = path.slice(dir.length + 1, -4);
    const tool = retrieveToolDefinition(() => ({ persistDir: dir, ttlMs: PERSIST_TTL_MS }));
    assert.equal(tool.name, "snap_retrieve");
    assert.match(tool.description, /handle/, "the schema explains what to pass");
    const hit = await tool.execute({ hash }, { signal: new AbortController().signal });
    assert.equal(hit, "the original bytes");
    const miss = await tool.execute({ hash: "ffffffffffff" }, { signal: new AbortController().signal });
    assert.match(miss, /no copy/, "a miss explains itself through the same channel");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
