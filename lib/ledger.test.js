...
import { excerptNotice, isPlaceholder, isRereadableTool, persistIngressText, snapExcerpt, collectText, tokenDigest, toolSourceLabel } from "./excerpt.js";
...
test("an unreadable result is written down so the notice names a reachable path", () => {
  ...
  assert.ok(shaped.text.includes(shaped.savedPath), "the notice names that path");
  assert.equal(readFileSync(shaped.savedPath, "utf8"), BIG_CMD, "the saved file is the original text");
  assert.equal(/cannot be re-read/.test(shaped.text), false, "the dead-end wording is gone once a path exists");
...
test("persistIngressText sanitizes the call id and writes the exact bytes", () => {
  const dir = mkdtempSync(join(tmpdir(), "ledger-persist-"));
  ...
  assert.equal(/[^A-Za-z0-9_.-]/.test(name), false, "the file name carries no separator characters");
  rmSync(dir, { recursive: true, force: true });
});
