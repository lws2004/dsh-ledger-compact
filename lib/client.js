    ...
    function SettingsSection(props) {
      ...
      const foldPane = h("div", { className: "dsh-ledger-tabpane" },
        ...
        h(SwitchRow, {
          icon: "pulse",
          label: "判定式折页",
          hint: "折页前问本机判定层（typed-decide）哪些结果仍需要且无法重跑，把它们的正文原样保留进卡片；判定层不可达、超时或置信不足时退回机械卡（真实会话上多数条目属此列）。",
          checked: value.decideFold === true,
          disabled: disabled,
          onChange: (checked) => write("decideFold", checked)
        }),
