...
import { verdictBodyLines, pickCandidates, decideEntries, describeVerdicts, summarizeVerdicts } from "./decide-fold.js";
...
export function inspectMessages(messages) {
  ...
  const errors = [];
  const excerptLines = [];
  /** Tool results that a verdict pass may restore to the card, largest usable first. */
  const entries = [];
  const carriedFiles = [];
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    ...
    if (result) {
      ...
      entries.push({
        ...
        // A run_code result has neither a path nor a shell command; its description is the
        // only name it has, and it is what later steps can be matched against.
        target: path || command || trim(pendingCall.args.description) || "",
        kind: EDIT_TOOLS.has(pendingCall.name) ? "edit" : (READ_TOOLS.has(pendingCall.name) ? "read" : "run"),
        text: resultText
      });
      continue;
    }
  ...
  const tools = [...toolCounts.entries()]
    ...
    .map(([name, n]) => ({ name, n }));
  return { intents, reads, edits, commands, errors, excerptLines, entries, tools, carriedFiles, carriedIntents, carriedErrors };
}
...
export function renderFold(snapshot, options = {}) {
  ...
  // A verdict pass names the results this card may not paraphrase: still needed, and
  // not reproducible by running the same call again. Their body goes back verbatim.
  const kept = [];
  for (const entry of (Array.isArray(snapshot.entries) ? snapshot.entries : [])) {
    const verdict = options.verdicts instanceof Map ? options.verdicts.get(entry.id) : options.verdicts?.[entry.id];
    const body = verdictBodyLines(entry, verdict, options);
    if (body.length === 0) continue;
    kept.push("  [" + entry.name + "] " + clampUtf8(entry.target || "(no target)", 160) + " — still needed, not reproducible by re-running:");
    kept.push(...body);
  }
  if (kept.length) sections.push("CARRIED DETAIL\n" + kept.join("\n"));
...
export function foldReport(messages, text, options = {}) {
  ...
  if (options.verdicts && options.verdicts.size > 0) payload.verdictFold = summarizeVerdicts(options.verdicts);
  const rows = [
  ...
  if (options.verdicts && options.verdicts.size > 0) rows.push(["verdict fold", describeVerdicts(options.verdicts)]);
...
/** The request the span was serving: the last user-authored message, ignoring injections. */
...
/**
 * Fold with a verdict pass in front of the card.
 *
 * The decider only chooses what the card must carry verbatim; it can never fail the fold.
 * If it is unreachable, slow, or unsure, every entry keeps the mechanical treatment, which
 * is exactly the card this plugin produced before the switch existed.
 */
export async function foldSummaryDecided(input, options = {}) {
  const messages = Array.isArray(input && input.messages) ? input.messages : [];
  let verdicts;
  try {
    const snapshot = inspectMessages(messages);
    const candidates = pickCandidates(snapshot.entries, options);
    const task = options.task || taskOf(messages);
    // No request, no verdict. Measured on three real sessions: an empty task drove all
    // twelve candidates to "drop" — byte-identical to what the decider answers when it is
    // handed no task at all. "Still needed to finish the current task" has no referent
    // without a task, so the pass does not run and every entry keeps the mechanical card.
    if (candidates.length > 0 && task) {
      verdicts = await decideEntries(candidates, { ...options, task });
    }
  } catch {
    verdicts = undefined;
  }
  const asked = verdicts && verdicts.size > 0;
  return foldSummary(input, asked ? { ...options, verdicts } : options);
}
