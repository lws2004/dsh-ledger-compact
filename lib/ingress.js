...
import { collectText, excerptNotice, isInstructionText, isPlaceholder, isRereadableTool, skipIngressTool, snapExcerpt, tokenDigest, toolSourceLabel } from "./excerpt.js";
...
export function shapeIngress(text, opts = {}) {
  ...
  const excerpt = snapExcerpt(text, { ...opts, rereadable });
...
export async function shapeCurrentTurnIngress(session, ctx, opts) {
  ...
  for (const { seq, event, message, result, callId } of pending) {
    ...
    const excerptText = excerptNotice(estTokensUtf8(text), snapExcerpt(text, { rereadable: isRereadableTool(toolName) }), "", tokenDigest(text), source);
