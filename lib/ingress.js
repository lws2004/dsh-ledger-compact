...
import { collectText, excerptNotice, isInstructionText, isPlaceholder, isRereadableTool, persistIngressText, skipIngressTool, snapExcerpt, tokenDigest, toolSourceLabel } from "./excerpt.js";
...
export function shapeIngress(text, opts = {}) {
  ...
  const savedPath = rereadable === false && opts.persistIngress === true
    ? persistIngressText(text, opts.persistDir, opts.callId)
    : undefined;
  const excerpt = snapExcerpt(text, { ...opts, rereadable, savedPath });
...
export async function shapeCurrentTurnIngress(session, ctx, opts) {
  ...
  for (const { seq, event, message, result, callId } of pending) {
    ...
    const source = toolSourceLabel(toolName, parseToolArgs(call?.args));
    ...
    const excerptText = excerptNotice(estTokensUtf8(text), snapExcerpt(text, { rereadable: isRereadableTool(toolName), savedPath: shaped.savedPath }), "", tokenDigest(text), source);
