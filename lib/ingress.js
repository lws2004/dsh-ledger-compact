...
import { collectText, excerptNotice, isInstructionText, isPlaceholder, skipIngressTool, snapExcerpt, tokenDigest, toolSourceLabel } from "./excerpt.js";
...
export async function shapeCurrentTurnIngress(session, ctx, opts) {
  ...
  for (const { seq, event, message, result, callId } of pending) {
    ...
    const excerptText = excerptNotice(estTokensUtf8(text), snapExcerpt(text), "", tokenDigest(text), source);
