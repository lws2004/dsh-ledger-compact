...
import { collectText, excerptNotice, isInstructionText, isPlaceholder, skipIngressTool, snapExcerpt, tokenDigest } from "./excerpt.js";
...
export function shapeIngress(text, opts = {}) {
  ...
  const excerptText = excerptNotice(tok, excerpt, "", digest);
  ...
  const notice = excerptNotice(tok, excerpt, extra, digest);
 ...
 */
function collectToolNames(session, callIds) {
  const names = new Map();
  if (!session || !callIds || callIds.size === 0) return names;
  const scan = (item) => {
    if (!item || item.type !== "tool/call") return;
    const callId = item.data?.callId;
    if (callId === undefined || !callIds.has(callId) || names.has(callId)) return;
    names.set(callId, String(item.data?.name ?? ""));
  };
...
export async function shapeCurrentTurnIngress(session, ctx, opts) {
  ...
  const toolNames = collectToolNames(session, callIds);

  for (const { seq, event, message, result, callId } of pending) {
    const text = collectText(result.content);
    const toolName = callId === undefined ? "" : (toolNames.get(callId) ?? "");
    ...
    const excerptText = excerptNotice(estTokensUtf8(text), snapExcerpt(text), "", tokenDigest(text));
