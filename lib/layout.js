...
export function buildGrid(lines, options) {
  const { columns, cellCols, gapCols, cols, gutterCols, gutterEvery } = options;
  const rows = [];
  let usedCells = 0;
  let totalSubRows = 0;
  let wrappedCells = 0;
  ...
  for (let i = 0; i < lines.length; i += columns) {
    const cells = [];
    let subRows = 1;
    for (let j = 0; j < columns && i + j < lines.length; j++) {
      const text = String(lines[i + j] ?? "");
      const parts = wrapCell(text, cellCols);
      cells.push(parts);
      if (parts.length > subRows) subRows = parts.length;
      if (parts.length > 1) wrappedCells += 1;
      placed += 1;
      usedCells += cellLength(text);
    }
    rows.push({ cells, subRows, startLine: i + 1 });
...
export function planGrid(lines, shape, options = {}) {
  ...
  const fixed = { cols: shape.cols, gutterCols, gutterEvery: cfg.gutterEvery, gapCols: cfg.gapCols };
  let best;
  let unguarded;
