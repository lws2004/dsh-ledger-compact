...
- **换字形（同尺寸）没有增益**：4 套字库实测——X.org 8x13 与 DejaVu/JetBrains 的 TTF 渲染。筛选轮 DejaVu Bold 领先 3 题，配对复核 60 样本 **46/60 vs 46/60** 打平。字形设计不改变有效像素数，编码器读的是像素不是笔法。字库生成器 `tools/make-atlas.py` 已入库，"更大的字"（10x16 配 10x18 格）随时一轮可验。
